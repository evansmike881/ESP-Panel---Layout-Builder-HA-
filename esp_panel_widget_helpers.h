...
static inline std::string esp_panel_icon_glyph(std::string icon_name, std::string type_name) {
  ...
  if (icon_name.find("thermometer") != std::string::npos || type_name == "temperature") return std::string("\U000F050F");
  if (icon_name.find("water-percent") != std::string::npos || type_name == "humidity") return std::string("\U000F058E");
  if (icon_name.find("ceiling-light") != std::string::npos || icon_name.find("lightbulb") != std::string::npos || type_name == "button") return std::string("\U000F0769");
  ...
  if (icon_name.find("door") != std::string::npos) return std::string("\U000F081C");
  if (icon_name.find("home") != std::string::npos) return std::string("\U000F07D0");
  if (icon_name.find("cog") != std::string::npos) return std::string("\U000F0493");
...
static inline EspPanelWidgetGeometry esp_panel_widget_geometry(
    int grid_w, int grid_h, const std::string &align, const std::string &type_name, int icon_scale, int label_scale, int value_scale) {
  ...
  const bool compact = card_w < 96 || card_h < 78 || grid_w == 1;
  const bool tall = card_h >= 148;
  const bool wide = card_w >= 228;
  const bool stacked = align == "center" || compact || tall;
  ...
  geometry.icon_x = end_aligned ? card_w - padding - icon_w : padding;
  geometry.label_x = end_aligned ? padding : padding + icon_w + text_gap;
  ...
  geometry.value_h = std::max(value_px + 8, card_h - geometry.value_y - padding);
  geometry.text_align_code = end_aligned ? 2 : 0;
  return geometry;
}
